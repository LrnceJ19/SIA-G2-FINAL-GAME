...
let savedTotalWrong = totalWrongList;
let savedTotalGames = totalGamesList;
...
window.submitData = function(getCoins, getTotalCorrect, getLevelsMastered, getEasy, getMedium, getHard, getScore, getBg, getTotalWrong, getTotalGames){
  savedCoins[accntNum] = getCoins;
  savedTotalCorrect[accntNum] = getTotalCorrect;
  savedLevelsMastered[accntNum] = getLevelsMastered;
  savedTotalWrong[accntNum] = getTotalWrong;
  savedTotalGames[accntNum] = getTotalGames;
...
window.getSecond = function(){
  for (let i of savedScore){
    if (i == second && (secondIndex != firstIndex || secondIndex != thirdIndex)){
      break;
    }
    secondIndex++;
  }
